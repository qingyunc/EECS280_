...
void compute_energy_matrix(const Image* img, Matrix* energy) {
  ...
  for (int r = 1; r < (Image_height(img) - 1); r++) {
    for (int c = 1; c < (Image_width(img) - 1); c++) {
      ...
      X = squared_difference(N, S) + squared_difference(W, E);
      *Matrix_at(energy, r, c) = X;
    }
  }
...
//           cost matrix is found and the seam array is filled with the c
//           numbers for each pixel along the seam, with the pixel for each
//           r r placed at seam[r]. While determining the seam, if any pixels
...
void find_minimal_vertical_seam(const Matrix* cost, int seam[]) {
  ...
  for (r = (Matrix_height(cost) - 2); r >= 0; r--) {
      c = seam[r+1];
      ...
      seam[r] = Matrix_column_of_min_value_in_row(cost, r, c_start, c_end);
  }
}
...
//           pixel will be removed from every r in the image. The pixel
//           removed from r r will be the one with c equal to seam[r].
...
void remove_vertical_seam(Image *img, const int seam[]) {
  ...
  Image_init(temp_img, (Image_width(img) - 1), Image_height(img));

  for (int r = 0; r < Image_height(img); r++) {
    for (int c = 0; c < Image_width(img); c++) {
      if (c < seam[r]) {
        Image_set_pixel(temp_img, r, c, Image_get_pixel(img, r, c));
      } else if (c > seam[r]) { 
        Image_set_pixel(temp_img, r, (c - 1), Image_get_pixel(img, r, c));
      }
    }
  }
...
void seam_carve_width(Image *img, int newWidth) {
  ...
  Matrix *temp_energy_mat = new Matrix;
  Matrix *temp_cost_mat = new Matrix;
  int seam[MAX_MATRIX_HEIGHT];

  while (Image_width(img) > newWidth) {
    compute_energy_matrix(img, temp_energy_mat);
    compute_vertical_cost_matrix(temp_energy_mat, temp_cost_mat);
    find_minimal_vertical_seam(temp_cost_mat, seam);
    remove_vertical_seam(img, seam);
  }

  delete temp_energy_mat;
  delete temp_cost_mat;
}

...
private:
  ...
  class PairComp {
  public:
    bool operator()(const Pair_type &p_1, const Pair_type &p_2) const {
      return Key_compare()(p_1.first, p_2.first);
    }
  };
...
public:
  ...
  //       using "Value_type()".
  Iterator find(const Key_type &k) const { return B_1.find({k, Value_type()}); }
  ...
  Value_type &operator[](const Key_type &k) {
    auto i_1 = B_1.find({k, Value_type()});
    if (i_1 != B_1.end()) {
      return i_1->second;
    } else {
      Pair_type p_3;
      p_3.first = k;
      p_3.second = Value_type();
      return B_1.insert(p_3)->second;
    }
  }
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val) {
    auto i_2 = B_1.find(val);
    if (i_2 != B_1.end()) {
      return {i_2, true};
    } else {
      auto it_1 = B_1.insert(val);
      return {it_1, false};
    }
  }

  // EFFECTS : Returns an iterator to the first key-value pair in this Map.
  Iterator begin() const { return B_1.begin(); }

  // EFFECTS : Returns an iterator to "past-the-end".
  Iterator end() const { return B_1.end(); }

private:
  // Add a BinarySearchTree private member HERE.
  BinarySearchTree<Pair_type, PairComp> B_1;
};
